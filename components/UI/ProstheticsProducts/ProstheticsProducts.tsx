...
export const ProstheticsProducts = () => {
  ...
  return (<Tabs itemWidth="equal" variant='pills-elevated'>
    <Tabs.List>
      {selectedToothData.map((x, index) => (
        <Tabs.Item value={`${index}`} key={index}>
          <View gap={3} direction='row' align='center'>
            <ImplantIcon />
            <Text variant='body-strong-2' color='neutral-faded'>{x.name}</Text>
          </View>
        </Tabs.Item>
      ))}
    </Tabs.List>
      ...
      {selectedToothData.map((data, index) => (
        <Tabs.Panel value={`${index}`} key={index}>
          <Tabs itemWidth="equal" variant='pills-elevated' >
            <Tabs.List>
              {data.attr.map((item, index) => (<Tabs.Item value={`${index}`} key={index}>
            ...
            <View paddingTop={2}>
              {data.attr.map((item, index) => (<Tabs.Panel value={`${index}`} key={index}>
                {item}
              </Tabs.Panel>))}
            </View>
          </Tabs>
